...
class RagEngine:
	...
	def _embed_query(self, question: str) -> list[float]:
		...
		logger.info(f"Question embedding complete in {elapsed:.2f}s.")
		return response["embedding"]

	def retrieve(self, question: str, top_k: int | None = None) -> list[RetrievedChunk]:
		k = top_k or self.config.top_k
		started_at = time.monotonic()
		logger.info(f"Retrieving top {k} chunk(s) from Qdrant...")
		vector = self._embed_query(question)

		try:
			logger.debug("Querying Qdrant with query_points...")
			...
			logger.warning("query_points unavailable; falling back to search...")
		...
		elapsed = time.monotonic() - started_at
		logger.info(f"Retrieved {len(retrieved)} chunk(s) in {elapsed:.2f}s.")
		return retrieved
