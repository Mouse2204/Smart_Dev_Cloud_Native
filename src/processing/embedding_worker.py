...
def _pdf_chunks(minio_client, config: AppConfig, object_name: str) -> list[ChunkPayload]:
	reader = _read_pdf_from_minio(minio_client, config, object_name)
	return _pdf_chunks_from_reader(reader, object_name, config)


def _pdf_chunks_from_path(pdf_path: Path, object_name: str, config: AppConfig) -> list[ChunkPayload]:
...
def run_pipeline(config: AppConfig, source_dir: Path | None, skip_upload: bool) -> dict[str, int]:
	if skip_upload:
	...
	try:
		...
		print(
			f"Warning: MinIO upload/indexing unavailable ({exc}). Falling back to local PDF indexing.",
			file=sys.stderr,
		)
...
def main() -> None:
	...
	try:
		summary = run_pipeline(config=config, source_dir=args.source_dir, skip_upload=args.skip_upload)
	...
	print(f" - Objects scanned: {summary['objects']}")
	print(f" - Text chunks: {summary['chunks']}")
	print(f" - Vector points upserted: {summary['points']}")
	print(f" - Objects skipped (unchanged): {summary['skipped']}")
	print(f" - Objects reindexed (changed): {summary['reindexed']}")
